...
static uint8_t dbg_binary[] = {
	...
	0xC0, 0x04, 0xC7, 0x00, 0x88, 0x77, 0x66, 0x55, 0xB8, 0x48, 0x10, 0x00,
	...
	0xC0, 0x33, 0xC0, 0xB8, 0x01, 0x20, 0x00, 0x00, 0x66, 0xC7, 0x00, 0x00,
	0x00, 0x83, 0xC0, 0x01, 0x66, 0x8B, 0x00, 0x0F, 0x20, 0xE0, 0x83, 0xE0,
	0xF7, 0x0F, 0x22, 0xE0, 0xB8, 0x00, 0x00, 0x00, 0x00, 0x0F, 0x23, 0xE0,
	0xB0, 0x02, 0x66, 0xBA, 0x23, 0x01, 0xEE, 0xFA, 0xF4, 0x0F, 0x21, 0xF0,
	0x83, 0xE0, 0x01, 0x83, 0xF8, 0x01, 0x0F, 0x85, 0x0E, 0x00, 0x00, 0x00,
	0x8B, 0x5C, 0x24, 0x08, 0x81, 0xCB, 0x00, 0x00, 0x01, 0x00, 0x89, 0x5C,
	0x24, 0x08, 0x8B, 0x1C, 0x24, 0x53, 0xE8, 0x1A, 0x00, 0x00, 0x00, 0x33,
	0xC0, 0x0F, 0x23, 0xF0, 0x83, 0xC4, 0x08, 0xCF, 0x8B, 0x1C, 0x24, 0x53,
	0xE8, 0x08, 0x00, 0x00, 0x00, 0x5B, 0x83, 0xC3, 0x02, 0x89, 0x1C, 0x24,
	0xCF, 0xC3
};
...
CDECL int_handler_printer(uint32_t eip)
{
	...
	case 0x1053:
		mem_read_block(cpu, 0x2000, 4, reinterpret_cast<uint8_t *>(&val));
		std::printf("data w breakpoint of 2 bytes at 0x%X: mem at 0x2000 should be 0xDEAD0000, it actually was 0x%08X\n", eip, val);
		break;

	case 0x105D:
		mem_read_block(cpu, 0x2000, 4, reinterpret_cast<uint8_t *>(&val));
		std::printf("data w breakpoint of 4 bytes at 0x%X: mem at 0x2000 should be 0x00000000, it actually was 0x%08X\n", eip, val);
		break;

	case 0x1068:
		mem_read_block(cpu, 0x2004, 4, reinterpret_cast<uint8_t *>(&val));
		std::printf("data r breakpoint of 4 bytes at 0x%X: mem at 0x2004 should be 0x55667788, it actually was 0x%08X\n", eip, val);
		break;
	...
	case 0x108B:
		...
		std::printf("undefined opcode exception at 0x%X: faulting opcode should be 0x0F23E0, it actually was 0x%X\n", eip, val >> 8);
		break;
	...
	case 0x10A4:
		std::printf("spurious data w breakpoint of 2 bytes at 0x%X\n", eip);
		break;

	case 0x10AC:
		std::printf("spurious data r breakpoint of 2 bytes at 0x%X\n", eip);
		break;
...
gen_dbg_test()
{
	...
	if (!LIB86CPU_CHECK_SUCCESS(hook_add(cpu, 0x1109, std::unique_ptr<hook>(new hook({ call_conv::x86_cdecl, call_conv::x86_cdecl,
